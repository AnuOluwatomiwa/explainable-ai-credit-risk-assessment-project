...
df_filename = "data/dataset_v1.xlsx"
model_filename="documentation/models_and_encoders/best_loan_model(SVC).pkl"
crs_filename="documentation/models_and_encoders/credit_risk_scaler.pkl"
ncs_filename="documentation/models_and_encoders/new_col_scaler.pkl"
ohe_filename="documentation/models_and_encoders/one_hot_encoder.pkl"
...
with center_col:
    ...
    coapplicant_income = st.number_input("Co-Applicant Income ($)", min_value=1.0)
    loan_amount = st.number_input("Loan Amount ($)", min_value=1.0)
    loan_amt_term = st.number_input("Loan Duration (IN MONTHS)", min_value=1, max_value=np.array(df.Loan_Amount_Term).max())
    ...
    if button:
        ...
        st.session_state['infos_df'] = infos_df
        ...
        st.session_state['credit_risk_scaler'] = credit_risk_scaler
        st.session_state['new_col_scaler'] = new_col_scaler
        
        st.session_state['model_info'] = engineered_df
